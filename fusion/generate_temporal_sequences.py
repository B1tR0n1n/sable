...
def generate_temporal_sequences(count: int, max_ticks: int = 12,
    ...
    # Output: list of sequences, each is a list of per-tick dicts
    ...
    # History for delayed observations
    ds_gt_history = {}
    ...
    while generated < count:
        ...
        # 12% of sequences: no injection — the system must recognize normal
        ...
        if not skip_injection and rng.random() < 0.30:
            ...
            osc_candidates = list(range(min(n, 15)))  # pick from first nodes
            if len(osc_candidates) >= n_osc:
                osc_nodes = set(np.random.RandomState(rng.randint(0, 2**31)).choice(
                    osc_candidates, size=n_osc, replace=False).tolist())

        # Initialize belief state for POMDP features
        belief = BeliefState(component_ids)

        # Tick-by-tick propagation with per-tick feature capture
        engine = PropagationEngine(max_ticks=max_ticks, soft_impact_factor=0.4 if use_deg else 0.3)

        tick_count = 0
        for tick in range(max_ticks):
            # Flip oscillating nodes at varied cadences (1-3 ticks)
            for oi in osc_nodes:
                ...
                if comp and tick > 0 and tick % rng.choice([1, 1, 2, 2, 3]) == 0:
                    if comp.state == ComponentState.HEALTHY:
                        ...
                        comp.health = 0.0
                    elif comp.state == ComponentState.FAILED:
            ...
            # Capture current state features
            ...
            # Ground truth states — base from sim, then detect oscillation
            gt_states = [STATE_MAP.get(graph.get_component(cid).state, 0) for cid in component_ids]

            # Detect oscillation: if a node's state flipped 2+ times in last 4 ticks
            if tick >= 2:
                for ci in range(n):
                    flips = 0
                    for dt in range(1, min(4, tick + 1)):
                        prev_key = (generated, tick - dt, component_ids[ci])
                        curr_key = (generated, tick - dt + 1, component_ids[ci]) if tick - dt + 1 < tick else None
            ...
            # Store gt history for delayed observations
            ...
            # Corrupt Mamba features: add noise to health values, mask some nodes
            mamba_tensor = torch.tensor(mamba_raw, dtype=torch.float32).reshape(n, -1)[:, :NODE_FEAT_DIM]
            # Add Gaussian noise to health (feature 0) — sigma=0.15
            health_noise = torch.randn(n) * 0.15
            mamba_tensor[:, 0] = (mamba_tensor[:, 0] + health_noise).clamp(0, 1)
            # 20% of nodes get masked health (set to 0.5 = uncertain)
            ...
            # Update belief with HARD fog-of-war:
            # - Only 30% of nodes observable per tick
            # - 15% chance of wrong observation (noise)
            # - 2-tick observation delay
            for i, cid in enumerate(component_ids):
                comp = graph.get_component(cid)
                if rng.random() < 0.30:  # 30% observable (was 70%)
                    state_name = str(comp.state).split(".")[-1].lower()
                    # 15% chance of noisy observation
                    if rng.random() < 0.15:
                        state_name = rng.choice(STATE_NAMES[:4])  # sim states only
                    # Use delayed state (2 ticks back) if available
                    if tick >= 2:
            ...
            if not changes and state.has_reached_steady_state(lookback=2):
                break
    ...
    # Save
    dataset = {
        "gnn": all_gnn,          # (count, max_ticks, MAX_NODES, GNN_DIM)
        "pomdp": all_pomdp,      # (count, max_ticks, MAX_NODES, POMDP_DIM)
        "mamba": all_mamba,       # (count, max_ticks, MAX_NODES, NODE_FEAT_DIM)
        "states": all_states,     # (count, max_ticks, MAX_NODES) long
        "mask": all_mask,         # (count, max_ticks, MAX_NODES)
        "seq_len": all_seq_len,   # (count,) — actual ticks per sequence
        "n_nodes": all_n_nodes,   # (count,) — nodes per topology
        "max_ticks": max_ticks,
        "max_nodes": MAX_NODES,
    }
